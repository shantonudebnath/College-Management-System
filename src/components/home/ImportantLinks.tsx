        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
          {links.map(link => (
            <a key={link.id} href={link.url} target="_blank" rel="noopener noreferrer"
              className="bg-white rounded-2xl p-5 text-center shadow-sm border border-gray-100 hover:shadow-md hover:border-purple-200 transition-all group">
              <div className="text-3xl mb-3">{link.icon ?? '🔗'}</div>
              <p className="text-xs font-semibold text-gray-800 group-hover:text-purple-700 leading-tight">{link.label}</p>
              <ExternalLink size={11} className="text-gray-300 group-hover:text-purple-400 mx-auto mt-2 transition-colors" />
            </a>
          ))}
        </div>
      </div>
