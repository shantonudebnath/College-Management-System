...
export default function TeacherAttendancePage() {
  ...
  return (
    <div>
      <DashboardHeader title="উপস্থিতি গ্রহণ" subtitle="শ্রেণিভিত্তিক দৈনিক উপস্থিতি" userName="Md. Shafiqul Islam" role="শিক্ষক" />
      <div className="p-6 space-y-5">
        {/* Controls */}
        <div className="bg-white rounded-2xl border border-gray-100 p-5 flex flex-wrap items-end gap-4">
          <div>
            <label className="text-xs font-semibold text-gray-600 block mb-1">শ্রেণি নির্বাচন</label>
            <select value={selectedClass} onChange={e => { setSelectedClass(e.target.value); setAttendance({}); setSaved(false); }}
              className="px-3 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm outline-none focus:border-purple-400">
              {MADRASHA_CLASSES.filter(c => ['class-8', 'class-9', 'class-10'].includes(c.id)).map(c => (
